...
while true; do
  choice=$(whiptail --title "$TITLE" --backtitle "$BACKTITLE" --menu "Choose an action" 22 90 12 \
    ...
    "8" "Recovery: purge/reinstall driver + fresh MOK" \
    "9" "Guided first-time flow" \
    "10" "Guided recovery flow" \
    "11" "Help" \
  ...
  case "$choice" in
    ...
    8)
      if confirm "Recovery will purge and reinstall Debian NVIDIA packages, then create a fresh MOK import. Continue?"; then
        run_script "Recovery" run_as_root "$PROJECT_DIR/scripts/25-recover.sh"
      fi
      ;;
    9) run_script "Guided first-time flow" "$PROJECT_DIR/scripts/run-all.sh" ;;
    10)
      if confirm "Guided recovery will purge/reinstall NVIDIA and start fresh MOK setup. Continue?"; then
